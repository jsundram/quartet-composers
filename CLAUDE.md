...
4. **`composers.json` and `readership.json` are generated by a four-stage pipeline; never
   ...
   reproducible. The last stage writes BOTH shipped files from one set of caches, and they must
...
- `scripts/ui-test.sh` — 157 behavioral checks against a real headless Chrome over CDP. It starts
