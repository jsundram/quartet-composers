...
the app must not ship a build step and this must not ship a JS runtime. make-story-svg.py IMPORTS
them from here rather than copying them a third time: the two cards may only drift from the page
together.
"""
import json
import math
...
VY_DOMAIN = (10, 260000)                             # EN readers / month, log -- as chart.js
...
# The key's two captions, as the page prints them: chart.js's DEFAULT_REPERTOIRE noun and the ring
# line in app.js's renderLegend(). og-lint.py's check_legend() reads both files and fails when
# these drift — this card said "the repertoire" for a week after the page stopped.
LEGEND = ("the notables", "some standouts")
...
def logscale(dom, px, origin, flip=False, clamp=True):
    """Clamped, as chart.js pins a dot under the floor to the edge. NOT for a diagonal's endpoints:
    clamping the 1-reader line's low end moved it to the corner and drew it at the wrong slope."""
    lo, hi = math.log10(dom[0]), math.log10(dom[1])
    def f(v):
        t = (math.log10(max(v, dom[0]) if clamp else v) - lo) / (hi - lo)
        return origin + (px - t * px if flip else t * px)
    return f


def clip(rect, ax, ay, bx, by):
    """Trim a segment to a plot rect (Liang-Barsky), same job as chart.js's trim(): a diagonal's
    endpoints are far outside the box and there is no clip path in a static SVG."""
    t0, t1, dx, dy = 0.0, 1.0, bx - ax, by - ay
    for p_, q_ in ((-dx, ax - rect["x"]), (dx, rect["x"] + rect["w"] - ax),
...
def main():
    ...
    ly = logscale(VY_DOMAIN, PLOT["h"], PLOT["y"], flip=True, clamp=False)
    ...
    # Filtered to the domain, as chart.js filters VY_TICKS. logscale() CLAMPS, so a tick under the
    # floor is not dropped but drawn on the bottom edge with the wrong number beside it.
    ...
    for k in RATIOS:
        seg = clip(PLOT, sx(QX_DOMAIN[0]), ly(k * QX_DOMAIN[0]), sx(QX_DOMAIN[1]), ly(k * QX_DOMAIN[1]))
        if seg:
